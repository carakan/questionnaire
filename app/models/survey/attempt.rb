...
class Survey::Attempt < ActiveRecord::Base
  ...
  scope :for_survey, ->(survey) { where(survey_id: survey.try(:id)) }
  scope :exclude_survey, ->(survey) { where("NOT survey_id = #{survey.try(:id)}") }
  scope :for_participant,
        lambda { |participant|
  ...
  def collect_scores
    multi_select_questions =
      Survey::Question.joins(:section).where(
        survey_sections: { survey_id: survey.id },
        survey_questions: { questions_type_id: Survey::QuestionsType.multi_select }
      )
    if multi_select_questions.empty? # No multi-select questions
      ...
      raw_score =
        answers.where.not(question_id: multi_select_questions.ids).map(&:value).reduce(:+) || 0
      multi_select_questions.each do |question|
        ...
        correct_percentage =
          correct_question_answers.map(&:value).reduce(:+).fdiv(correct_options_sum)
        raw_score += correct_percentage
        next unless correct_percentage == 1

        option_value = 1 / options.count.to_f
        raw_score -=
          (option_value * answers.where(question_id: question.id, correct: false).count)
      end
