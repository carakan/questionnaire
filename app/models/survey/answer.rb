...
class Survey::Answer < ActiveRecord::Base
  ...
  belongs_to :attempt
  belongs_to :option
  belongs_to :predefined_value, required: false
  belongs_to :question
  ...
  validates :predefined_value_id,
            ...
            if:
              proc { |a|
                a.question && a.question.mandatory? && a.question.predefined_values.count > 0 &&
                  ![Survey::OptionsType.text, Survey::OptionsType.large_text].include?(
                    a.option.options_type_id
                  )
              }
  validates :option_text,
            ...
            if:
              proc { |a|
                a.option &&
                  (
                    a.question && a.question.mandatory? &&
                      a.question.predefined_values.count == 0 &&
                      [
                        Survey::OptionsType.text,
                        Survey::OptionsType.multi_choices_with_text,
                        Survey::OptionsType.single_choice_with_text,
                        Survey::OptionsType.large_text
                      ].include?(a.option.options_type_id)
                  )
              }
  ...
  def value
    option.nil? ? Survey::Option.find(option_id).weight : option.weight
  end
  ...
  def check_single_choice_with_field_case
    if [Survey::OptionsType.multi_choices, Survey::OptionsType.single_choice].include?(
      option.options_type_id
    )
