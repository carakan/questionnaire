...
ActiveAdmin.register Survey::Survey do
  ...
  filter :name,
         as: :select,
         collection:
           proc { Survey::Survey.select('distinct(name)').collect { |c| [c.name, c.name] } }
  filter :active, as: :select, collection: %w[true false]
  filter :created_at
    ...
    f.inputs I18n.t('sections') do
      f.has_many :sections do |s|
        ...
        s.has_many :questions do |q|
          ...
          q.input :questions_type_id,
                  as: :select, collection: Survey::QuestionsType.questions_types_title
          q.input :mandatory
          ...
          q.has_many :options do |a|
            ...
            a.input :text
            a.input :locale_text
            a.input :options_type_id,
                    as: :select, collection: Survey::OptionsType.options_types_title
            a.input :correct
          end
        end
      end
